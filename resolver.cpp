...
void Resolver::SetMode(LagRecord* record) {
	...
	else if ((record->m_flags & FL_ONGROUND) && (speed <= 0.1f || record->m_fake_walk) && !g_input.GetKeyState(g_menu.main.aimbot.resolver_override.get()))
...
void Resolver::ResolveAngles(Player* player, LagRecord* record) {
	...
	else if (record->m_mode == Modes::RESOLVE_LASTMOVE || record->m_mode == Modes::RESOLVE_UNKNOWM && !(g_input.GetKeyState(g_menu.main.aimbot.resolver_override.get())))
