...
void events::round_start(IGameEvent* evt) {
	...
	// fix fix?
	core.m_body_pred = g_csgo.m_globals->m_curtime;
...
void events::round_end(IGameEvent* evt) {
	...
	if (core.m_local->m_iTeamNum() == TEAM_COUNTERTERRORISTS && reason == CSRoundEndReason::CT_WIN)
		core.m_round_end = true;

	else if (core.m_local->m_iTeamNum() == TEAM_TERRORISTS && reason == CSRoundEndReason::T_WIN)
		core.m_round_end = true;
}
...
void events::player_hurt(IGameEvent* evt) {
	...
	if (evt) {
		...
		if (!attacker || !victim || attacker != core.m_local)
			return;
...
void events::bullet_impact(IGameEvent* evt) {
	if (core.m_processing) {
		...
		if (!shooter || shooter != core.m_local)
			return;
...
void events::item_purchase(IGameEvent* evt) {
	...
	if (team == core.m_local->m_iTeamNum())
		return;
