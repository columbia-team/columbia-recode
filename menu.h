...
public:
	void init() {
		...
		lagfix.setup(XOR("fake-lag correction"), XOR("lagfix"));
		...
		head1.setup(XOR("prefer head"), XOR("head1"), { XOR("always"), XOR("running"), XOR("resolved"), XOR("in air") });
		...
		//fakeduck.SetToggleCallback(callbacks::FakeDuck);
...
public:
	void init() {
		...
		body_fake_stand.setup(XOR("fake body"), XOR("body_fake_stnd"), { XOR("off"), XOR("left"), XOR("right"), XOR("opposite"), XOR("z"), XOR("twist"), XOR("twist z"), XOR("fick random") });
		...
		// col2.
		fake_yaw.setup(XOR("fake yaw"), XOR("fake_yaw"), { XOR("off"), XOR("default"), XOR("relative"), XOR("jitter"), XOR("rotate"), XOR("random"), XOR("local view"), XOR("flick angle") });
		...
		lag_active.setup("", XOR("lag_active"), { XOR("stand"), XOR("move"), XOR("air"), XOR("crouch"), XOR("reloading"), XOR("weapon switch"), XOR("always") }, false);
...
public:
	void init() {
		...
		health_enemy.AddShowCallback(callbacks::ENEMY_SELECTION);
		...
		col_skeleton_enemy.AddShowCallback(callbacks::ENEMY_SELECTION);
		...
		weapon_enemy.AddShowCallback(callbacks::ENEMY_SELECTION);
		RegisterElement(&weapon_enemy);

		weapon_mode.setup("", XOR("weapon_mode"), { XOR("text"), XOR("icon") }, false);
		weapon_mode.AddShowCallback(callbacks::ENEMY_SELECTION);
		RegisterElement(&weapon_mode);

		ammo.setup(XOR("ammo"), XOR("ammo"));
		ammo.AddShowCallback(callbacks::ENEMY_SELECTION);
		RegisterElement(&ammo);

		ammo_color.setup(XOR("color"), XOR("ammo_color"), colors::burgundy);
		ammo_color.AddShowCallback(callbacks::ENEMY_SELECTION);
		RegisterElement(&ammo_color);

		lby_update.setup(XOR("lby information"), XOR("lby_update"));
		lby_update.AddShowCallback(callbacks::ENEMY_SELECTION);
		RegisterElement(&lby_update);

		lby_update_color.setup(XOR("color"), XOR("lby_update_color"), colors::orange);
		lby_update_color.AddShowCallback(callbacks::ENEMY_SELECTION);
...
public:
	void init() {
		...
		proj_color.setup(XOR("projectiles color"), XOR("proj_color"), colors::white);
...
public:
	...
	Keybind  cripwalk;
	Keybind  quickstop;
	Checkbox quickstop_always_on;
	//Dropdown autostop_mode;
...
public:
	void init() {
		...
		//autopeek_color_blend.setup("", XOR("blend"), 10.f, 100.f, false, 0, 100.f, 1.f, XOR(L"%"));
		//RegisterElement(&autopeek_color_blend, 1);
		...
		quickstop_always_on.setup(XOR("automatic stop always on"), XOR("quickstop_always"));
		RegisterElement(&quickstop_always_on, 1);

		quickstop.setup(XOR("automatic stop (key)"), XOR("quickstop"));
		quickstop.AddShowCallback(callbacks::AUTO_STOP);
		RegisterElement(&quickstop, 1);

		//autostop_mode.setup(XOR("automatic stop mode"), XOR("quickstop_mode"), { XOR("full stop"), XOR("fakewalk") });
		//RegisterElement(&autostop_mode, 1);
	}
};
...
public:
	void init() {
		...
		//aspect.setup(XOR("aspect ratio"), XOR("aspect"), 0.1f, 2, true, 1, 1.6f, 0.1f);
		//RegisterElement(&aspect, 1);

		hitsound.setup(XOR("hitsound"), XOR("hitsound"));
		RegisterElement(&hitsound);

		hitsound_volume.setup(XOR("hitsound volume"), XOR("hitsound_volume"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
