...
void GUI::think( ) {
	...
	math::clamp( g_input.m_mouse.x, 0, core.m_width );
	math::clamp( g_input.m_mouse.y, 0, core.m_height );
				...
				if ( g_input.IsCursorInRect( cl ) ) {
					...
					if ( g_input.IsCursorInRect( tabs_area ) && !f->m_tabs.empty( ) ) {
						...
						for ( size_t i{}; i < f->m_tabs.size( ); ++i ) {
							...
							Rect tab{ ( tabs_area.x + render::menu_shade.size( "colubmia" ).m_width + 20 ) + font_w, tabs_area.y + 11, 140, tabs_area.h };
