...
void Skins::think() {
	...
	player_info_t info;
	if (!g_csgo.m_engine->GetPlayerInfo(core.m_local->index(), &info))
		return;
		...
		if (g_menu.main.skins.knife.get() > 0 && ent->is(HASH("CPredictedViewModel"))) {
			...
			if (weapon->IsKnife()) {
				...
				if (g_menu.main.skins.knife.get() > 0) {
					...
					// fix definition.
					if (core.m_local->m_iTeamNum() == TEAM_TERRORISTS)
						weapon->m_iItemDefinitionIndex() = KNIFE_T;

					else if (core.m_local->m_iTeamNum() == TEAM_COUNTERTERRORISTS)
	...
	if (g_menu.main.skins.glove.get() > 0) {
		CBaseHandle* wearables = core.m_local->m_hMyWearables();
