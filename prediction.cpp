...
bool Hooks::InPrediction() {
	...
	if (core.m_local && g_menu.main.visuals.novisrecoil.get()) {
		...
		if (stack.ReturnAddress() == CalcPlayerView_ret2) {
			...
			if (angles) {
				*angles -= core.m_local->m_viewPunchAngle()
					+ (core.m_local->m_aimPunchAngle() * g_csgo.weapon_recoil_scale->GetFloat())
