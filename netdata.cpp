...
void NetData::store( ) {
    ...
    tickbase = core.m_local->m_nTickBase( );

    // get current record and store data.
    data = &m_data[ tickbase % MULTIPLAYER_BACKUP ];
    
    data->m_tickbase    = tickbase;
    data->m_punch       = core.m_local->m_aimPunchAngle( );
    data->m_punch_vel   = core.m_local->m_aimPunchAngleVel( );
	data->m_view_offset = core.m_local->m_vecViewOffset( );
}
...
void NetData::apply( ) {
    ...
    if( core.m_local->m_nTickBase( ) != data->m_tickbase )
    	return;
    ...
    punch_delta     = core.m_local->m_aimPunchAngle( ) - data->m_punch;
    punch_vel_delta = core.m_local->m_aimPunchAngleVel( ) - data->m_punch_vel;
	view_delta      = core.m_local->m_vecViewOffset( ) - data->m_view_offset;
    
    // set data.
    if( std::abs( punch_delta.x ) < 0.03125f &&
    	std::abs( punch_delta.y ) < 0.03125f &&
    	std::abs( punch_delta.z ) < 0.03125f )
    	core.m_local->m_aimPunchAngle( ) = data->m_punch;
    
    if( std::abs( punch_vel_delta.x ) < 0.03125f &&
    	std::abs( punch_vel_delta.y ) < 0.03125f &&
    	std::abs( punch_vel_delta.z ) < 0.03125f )
    	core.m_local->m_aimPunchAngleVel( ) = data->m_punch_vel;

	if( std::abs( view_delta.x ) < 0.03125f &&
		std::abs( view_delta.y ) < 0.03125f &&
		std::abs( view_delta.z ) < 0.03125f )
		core.m_local->m_vecViewOffset( ) = data->m_view_offset;
}
