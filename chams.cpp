...
bool Chams::OverridePlayer(int index) {
	...
	bool enemy = core.m_local && player->enemy(core.m_local);
...
bool Chams::GenerateLerpedMatrix(int index, BoneArray* out) {
	...
	for (auto it = data->m_records.rbegin(); it != data->m_records.rend(); it++) {
		...
		if (current_record && current_record->valid() && (!end && ((it + 1)->get()))) {
			...
			float correct = total_latency + core.m_lerp;
...
void Chams::RenderPlayer(Player* player) {
	...
	if (player->m_bIsLocalPlayer()) {
		...
		if (g_menu.main.players.chams_local_scope.get() && core.m_weapon_id == HEGRENADE) //super ghetto way of doing transparent on grenade
			SetAlpha(0.5f);

		if (g_menu.main.players.chams_local_scope.get() && core.m_weapon_id == MOLOTOV)
			SetAlpha(0.5f);

		if (g_menu.main.players.chams_local_scope.get() && core.m_weapon_id == SMOKE)
			SetAlpha(0.5f);

		if (g_menu.main.players.chams_local_scope.get() && core.m_weapon_id == FLASHBANG)
			SetAlpha(0.5f);

		if (g_menu.main.players.chams_local_scope.get() && core.m_weapon_id == DECOY)
			SetAlpha(0.5f);
	...
	else if (!enemy && !core.m_local && g_menu.main.players.chams_friendly.get(0)) {
...
bool Chams::SortPlayers() {
	...
	static auto distance_predicate = [](Entity* a, Entity* b) {
		vec3_t local = core.m_local->GetAbsOrigin();
