...
void c_grenade_prediction::on_create_move(CUserCmd* cmd) {
    ...
    if (!core.m_processing || !g_menu.main.visuals.tracers.get())
        return;
    ...
    math::AngleVectorKidua(view_angles, direction);
...
void DrawBeamPaw(vec3_t src, vec3_t end, Color color)
{
    ...
    beamInfo.m_flRed = g_menu.main.visuals.warningtracer_col.get().r();
    beamInfo.m_flGreen = g_menu.main.visuals.warningtracer_col.get().g();
    beamInfo.m_flBlue = g_menu.main.visuals.warningtracer_col.get().b();
...
bool c_grenade_prediction::data_t::draw() const
{
    ...
    auto dist_world = core.m_local->m_vecOrigin().dist_to(m_origin);
    if (dist_world > 150.f) {
        col1.a() *= std::clamp((750.f - (dist_world - 200.f)) / 750.f, 0.f, 1.f);
    }

    Color col = colors::white;
    col.a() = 0xb4;

    auto dist_world1 = core.m_local->m_vecOrigin().dist_to(m_origin);
    if (dist_world1 > 150.f) {
        col.a() *= std::clamp((750.f - (dist_world - 200.f)) / 750.f, 0.f, 1.f);
    }

    if (dist < 25) {
        render::circle(prev_screen.x, prev_screen.y - 10, 20, 360, Color(15, 15, 15, col1.a()));
        draw_arc(prev_screen.x, prev_screen.y - 10, 20, 0, 360 * percent, 2, col1);
        render::icons1.string(prev_screen.x - 6, prev_screen.y - 19, col1, index_to_grenade_name_icon(m_index));
    }
    else  if (dist < 150) {
        render::circle(prev_screen.x, prev_screen.y - 10, 20, 360, Color(15, 15, 15, col.a()));
        draw_arc(prev_screen.x, prev_screen.y - 10, 20, 0, 360 * percent, 2, col);
        render::icons1.string(prev_screen.x - 6, prev_screen.y - 19, col, index_to_grenade_name_icon(m_index));
    }
    ...
    if (!is_on_screen(vEnemyOrigin, screenPos))
    {
        ...
        if (dist < 45) {
            render::circle(position.x, position.y - 10, 20, 360, Color(15, 15, 15, col.a()));
            draw_arc(position.x, position.y - 10, 20, 0, 360 * percent, 2, col);
            render::icons1.string(position.x - 6, position.y - 19, col, index_to_grenade_name_icon(m_index));
        }
