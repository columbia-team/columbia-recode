...
void Grenades::think() {
	...
	m_vel = core.m_weapon_info->m_throw_velocity;
...
void Grenades::simulate() {
	...
	if (m_id == MOLOTOV || m_id == FIREBOMB) {
		CGameTrace trace;
		PhysicsPushEntity(m_start, { 0.f, 0.f, -131.f }, trace, core.m_local);
...
void Grenades::setup() {
	...
	ang_t angle = core.m_view_angles;
	...
	CGameTrace trace;
	TraceHull(m_start, end, trace, core.m_local);
	...
	// weird formula, valve..
	m_velocity = core.m_local->m_vecVelocity();
	m_velocity *= 1.25f;
...
size_t Grenades::advance(size_t tick) {
	...
	// move object.
	PhysicsPushEntity(m_start, m_move, trace, core.m_local);
...
void Grenades::ResolveFlyCollisionBounce(CGameTrace& trace) {
	...
	if (trace.m_plane.m_normal.z > 0.7f) {
		...
		if (speed < 400.f)
			...
			// advance forward.
			PhysicsPushEntity(trace.m_endpos, velocity * (left * g_csgo.m_globals->m_interval), trace, core.m_local);
		}
	}
