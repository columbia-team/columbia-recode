...
bool CanFireWithExploit(int m_iShiftedTick)
{
	...
	float curtime = game::TICKS_TO_TIME(core.m_local->m_nTickBase() - m_iShiftedTick);
	return core.CanFireWeapon(curtime);
}

bool Aimbot::CanDT() {
	int idx = core.m_weapon->m_iItemDefinitionIndex();
	return core.m_local->alive()
...
void Aimbot::DoubleTap()
{
	...
	if (CanDT())
	{
		if (m_double_tap)
		{
			...
			{
				if (core.m_weapon->DTable() && CanFireWithExploit(prev_shift_ticks))
				{
					if (core.m_cmd->m_buttons & IN_ATTACK)
					{
						core.m_tick_to_shift = prev_shift_ticks;
						reset = true;
					}
					else {
						if ((!(core.m_cmd->m_buttons & IN_ATTACK) || !core.m_shot) && reset
							&& fabsf(core.m_weapon->m_fLastShotTime() - game::TICKS_TO_TIME(core.m_local->m_nTickBase())) > 0.5f) {
							core.m_charged = false;
							core.m_tick_to_recharge = shift_ticks;
							reset = false;
						}
					}
				}
...
void AimPlayer::OnNetUpdate(Player* player) {
	bool reset = (!g_menu.main.aimbot.enable.get() || player->m_lifeState() == LIFE_DEAD || !player->enemy(core.m_local));
	bool disable = (!reset && !core.m_processing);
...
void Aimbot::StripAttack() {
	if (core.m_weapon_id == REVOLVER)
		core.m_cmd->m_buttons &= ~IN_ATTACK2;

	else
		core.m_cmd->m_buttons &= ~IN_ATTACK;
}
...
void Aimbot::think() {
	...
	// no grenades or bomb.
	if (core.m_weapon_type == WEAPONTYPE_GRENADE || core.m_weapon_type == WEAPONTYPE_C4)
		return;

	if (!core.m_weapon_fire)
		StripAttack();
	...
	bool revolver = core.m_weapon_id == REVOLVER && core.m_revolver_cock != 0;

	// one tick before being able to shoot.
	if (revolver && core.m_revolver_cock > 0 && core.m_revolver_cock == core.m_revolver_query) {
		*core.m_packet = false;
		return;
	}
	...
	if (core.m_weapon_fire && !core.m_lag && !revolver) {
		*core.m_packet = false;
		StripAttack();
		return;
	}
	...
	// run knifebot.
	if (core.m_weapon_type == WEAPONTYPE_KNIFE && core.m_weapon_id != ZEUS) {
...
void Aimbot::find() {
	...
	if (core.m_weapon_id == ZEUS && !g_menu.main.aimbot.zeusbot.get())
		return;
	...
	if (best.player && best.record) {
		// calculate aim angle.
		math::VectorAngles(best.pos - core.m_shoot_pos, m_angle);
		...
		// set autostop shit.
		m_stop = !(core.m_buttons & IN_JUMP);

		bool can_hit_on_fd = !g_hvh.m_fakeduck || g_hvh.m_fakeduck && core.m_local->m_flDuckAmount() == 0.f;

		bool on = g_menu.main.aimbot.hitchance.get() && g_menu.main.config.mode.get() == 0;
		bool hit = on && CheckHitchance(m_target, m_angle);

		// if we can scope.
		bool can_scope = !core.m_local->m_bIsScoped() && (core.m_weapon_id == AUG || core.m_weapon_id == SG553 || core.m_weapon_type == WEAPONTYPE_SNIPER_RIFLE);
		...
		if (hit || !on) {
			// right click attack.
			if (g_menu.main.config.mode.get() == 1 && core.m_weapon_id == REVOLVER)
				core.m_cmd->m_buttons |= IN_ATTACK2;
...
bool Aimbot::SelectTarget(LagRecord* record, const vec3_t& aim, float damage) {
	...
	if (g_menu.main.aimbot.fov.get()) {
		...
		if (math::GetFOV(core.m_view_angles, core.m_shoot_pos, aim) > g_menu.main.aimbot.fov_amount.get())
			return false;
	}
	...
	switch (g_menu.main.aimbot.selection.get()) {
		...
		// distance.
	case 0:
		dist = (record->m_pred_origin - core.m_shoot_pos).length();
		...
		// crosshair.
	case 1:
		fov = math::GetFOV(core.m_view_angles, core.m_shoot_pos, aim);
	...
	case 4:
		...
		// height.
	case 5:
		height = record->m_pred_origin.z - core.m_local->m_vecOrigin().z;
...
void Aimbot::apply() {
	...
	attack2 = (core.m_weapon_id == REVOLVER && core.m_cmd->m_buttons & IN_ATTACK2);
	...
	if (attack || attack2) {
		...
		if (m_target) {
			...
			if (m_record && !m_record->m_broke_lc)
				core.m_cmd->m_tick = game::TIME_TO_TICKS(m_record->m_sim_time + core.m_lerp);

			// set angles to target.
			core.m_cmd->m_view_angles = m_angle;
		...
		if (g_menu.main.aimbot.norecoil.get())
			core.m_cmd->m_view_angles -= core.m_local->m_aimPunchAngle() * g_csgo.weapon_recoil_scale->GetFloat();

		// store fired shot.
		g_shots.OnShotFire(m_target ? m_target : nullptr, m_target ? m_damage : -1.f, core.m_weapon_info->m_bullets, m_target ? m_record : nullptr, m_hitbox);
...
void Aimbot::NoSpread() {
	...
	// get spread.
	spread = core.m_weapon->CalculateSpread(core.m_cmd->m_random_seed, attack2);

	// compensate.
	core.m_cmd->m_view_angles -= { -math::rad_to_deg(std::atan(spread.length_2d())), 0.f, math::rad_to_deg(std::atan2(spread.x, spread.y)) };
}