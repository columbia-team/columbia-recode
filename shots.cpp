...
void Shots::OnShotFire(Player* target, float damage, int bullets, LagRecord* record, int hitbox) {
	...
	for (int i{ }; i < bullets; ++i) {
		...
		shot.m_time = game::TICKS_TO_TIME(core.m_local->m_nTickBase());
		shot.m_lat = core.m_latency;
		shot.m_damage = damage;
		shot.m_hitbox = hitbox;
		shot.m_pos = core.m_local->GetShootPosition();
...
void Shots::OnImpact(IGameEvent* evt) {
	...
	time = game::TICKS_TO_TIME(core.m_local->m_nTickBase());
	...
	if (g_menu.main.visuals.impact_beams.get())
		m_vis_impacts.push_back({ pos, core.m_local->GetShootPosition(), core.m_local->m_nTickBase() });
	...
	ImpactRecord impact;
	impact.m_shot = shot;
	impact.m_tick = core.m_local->m_nTickBase();
	impact.m_pos = pos;
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	// hitsound.
	// todo: simv0l - use BASS API for hitsound(s).
	...
	if (g_menu.main.misc.notifications.get(1)) {
		...
		if (hp == 0 && g_menu.main.misc.killsay.get()) {
			...
			switch (killsay) {
			case 0:
				if (damage > 99) // if headshot or awp 1 baim
					g_csgo.m_engine->ExecuteClientCmd("say 1 blasted lachkick HAHAHAHAHAHAHAHA");
				else
					g_csgo.m_engine->ExecuteClientCmd("say 1 blasted lachkick HAHAHAHAHAHAHAHA");
				break;
			...
			case 1:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say HEADSHOT lachflip");
				else
					g_csgo.m_engine->ExecuteClientCmd("say HEADSHOT lachflip");
				break;
			...
			case 2:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say hhhhhhh 1 shot by the NNblaster");
				else
					g_csgo.m_engine->ExecuteClientCmd("say hhhhhhh 1 shot by the NNblaster");
				break;
			...
			case 3:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say laff you suck HAHAHAHAHA");
				else
					g_csgo.m_engine->ExecuteClientCmd("say laff you suck HAHAHAHAHA");
				break;
			...
			case 4:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say GO KYS");
				else
					g_csgo.m_engine->ExecuteClientCmd("say GO KYS");
				break;
			}
	...
	if (!m_vis_impacts.empty()) {
		for (auto& i : m_vis_impacts) {
			if (i.m_tickbase == core.m_local->m_nTickBase())
				i.m_hit_player = true;
		}
	}
	...
	for (auto& i : m_impacts) {
		...
		if (i.m_tick != core.m_local->m_nTickBase())
			continue;
