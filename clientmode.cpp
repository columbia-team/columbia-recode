...
void Hooks::OverrideView( CViewSetup* view ) {
	...
	if( g_menu.main.visuals.noscope.get( ) ) {
		if( core.m_local && core.m_local->m_bIsScoped( ) )
            view->m_edge_blur = 0;
	}
}
...
/*bool Hooks::CreateMove( float time, CUserCmd* cmd ) {
	...
	// call EngineCore::SetViewAngles according to what the original returns.
	if( ret )
	...
	core.m_packet = stack.next( ).local( 0x1c ).as< bool* >( );

	// get bFinalTick off the stack.
	core.m_final_packet = stack.next( ).local( 0x1b ).as< bool* >( );
