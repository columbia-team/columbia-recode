...
int Hooks::SendDatagram(void* data) {
	if (!this || !g_csgo.m_engine->IsInGame() || !g_csgo.m_net) // test
		return g_hooks.m_net_channel.GetOldMethod< SendDatagram_t >(INetChannel::SENDDATAGRAM)(this, data); // test
	...
	if (g_aimbot.m_fake_latency) {
		...
		// the target latency.
		float correct = std::max(0.f, (ping / 1000.f) - core.m_latency - core.m_lerp);
