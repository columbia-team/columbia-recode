...
ulong_t __stdcall Core::init(void* arg) {
	core.name = XOR("columbia"); //hack name for configs etc.
...
void Core::Watermark() {
	...
	render::gradient(m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 4, ciocan, Color(0, 0, 0, 0));
	render::rect(m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 4, ciocan);

	// text.
	render::hud.string(m_width - size.m_width - 15, 12, Color(255, 255, 255), text, render::ALIGN_LEFT);
}
...
void Core::ClanTag()
{
	...
	static int iOldTickCountPred = 0;
	static bool bReset = false;

	float flLatency = m_latency / g_csgo.m_globals->m_interval;
	float flTickCountPred = g_csgo.m_globals->m_tick_count + flLatency;

	if (g_menu.main.misc.clantag.get()) {
		static auto is_freeze_period = false;
		if (g_csgo.m_gamerules->m_bFreezePeriod() == true) {
			if (is_freeze_period)
			{
				if (iOldTickCountPred != int(flTickCountPred)) {
					if (&Core::ClanTag)
						SetClanTag(XOR("columbia"));
				}
				iOldTickCountPred = int(flTickCountPred);
			}
		...
		if (&Core::ClanTag) {
			if (iOldTickCountPred != int(flTickCountPred / 19) % 31) {
				switch (int(flTickCountPred / 19) % 31) {
				case 0: {SetClanTag(XOR("c")); break; }
				case 1: {SetClanTag(XOR("co")); break; }
				case 2: {SetClanTag(XOR("col")); break; }
				case 3: {SetClanTag(XOR("colu")); break; }
				...
				case 11: {SetClanTag(XOR("lumbia")); break; }
				case 12: {SetClanTag(XOR("umbia")); break; }
				case 13: {SetClanTag(XOR("mbia")); break; }
				case 14: {SetClanTag(XOR("bia")); break; }
				case 15: {SetClanTag(XOR("ia")); break; }
				case 16: {SetClanTag(XOR("a")); break; }
				case 17: {SetClanTag(XOR(" ")); break; }
				case 18: {SetClanTag(XOR("")); break; }
				}
				iOldTickCountPred = int(flTickCountPred / 19) % 31;
			}
		}
...
void Core::StartMove(CUserCmd* cmd) {
	...
	m_tickbase = m_fixed_tickbase = m_local->m_nTickBase();
...
void Core::UpdateAnimations() {
	...
	auto ang = ang_t(0.f, 0.f, 0.f);

	static auto wasOnGround = true;

	const auto player_flags = core.m_local->m_fFlags();

	if (!wasOnGround || !(core.m_local->m_fFlags() & FL_ONGROUND))
		core.m_local->m_fFlags() &= ~FL_ONGROUND;

	if (!g_csgo.m_cl->m_choked_commands)
		wasOnGround = player_flags & FL_ONGROUND;

	const auto inLandAnim = static_cast<bool>(core.m_local->m_PlayerAnimState()->m_land);
	const auto onGround = core.m_local->m_fFlags() & FL_ONGROUND;

	if (inLandAnim && onGround && wasOnGround) {
		ang.x = -10.f;
		ang.y = core.m_local->m_PlayerAnimState()->m_goal_feet_yaw;
	}

	core.m_local->m_fFlags() = player_flags;
	core.m_local->GetRenderAngles() = ang;
}
